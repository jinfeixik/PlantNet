...
$(document)
		.ready(
				function() {
					...
					$('#account_input').bind('focus', function() {
						$(this).attr('class', 's-txt-focus')
					});
					$('#account_input').bind('blur', function() {
						$(this).attr('class', 's-txt')
					});

					$('#pwd_input').bind('focus', function() {
						$(this).attr('class', 's-txt-focus')
					});
					$('#pwd_input').bind('blur', function() {
						$(this).attr('class', 's-txt')
					});
					...
					var loginCallback = function(i) {
						if (i == "login") {
							$('#js_login_input').html('<p>' + '登陆成功' + '</p>')
									.show();
							window.location.href = U('http://localhost:8080/PlantNet/index.jsp');
						} else {
							$('#js_login_input').html('<p>' + '登陆失败' + '</p>')
									.show();
						}
					};
				});
	...
	objInput.bind({
		focus : function() {
			...
			typeof (blurObjInput) != "undefined"
					&& loginCheck
							.bindKeyEvent(objInput, blurObjInput, objLabel);
		},
		keydown : function() {
			...
			.extend({
				changeTips : function(value) {
					...
					var keyChange = function(up) {
						if (up == "up") {
							if (indexLi == 0) {
								indexLi = $(value.divTip)
										.find('li[rel="show"]').length - 1;
							} else {
								indexLi--;
							}
						} else {
							if (indexLi == $(value.divTip).find(
									'li[rel="show"]').length - 1) {
								indexLi = 0;
							} else {
								indexLi++;
							}
						}
						$(value.divTip).find('li[rel="show"]').eq(indexLi)
					...
					var valChange = function() {
						...
						if ($this.val() == "") {
							...
							$(value.divTip)
									...
									.each(
											function(index) {
												var valAttr = $(this).attr(
														"email");
												if (index == 0) {
													$(this).text(tex).addClass(
															'current')
															.siblings()
															.removeClass();
												}
					...
					$this.keydown(function(event) {
						if (event.which == 38) {
							...
							// 按回车
							var liVal = $(value.divTip).find('li[rel="show"]')
									.eq(indexLi).text();
							$this.val(liVal);
							blus();
							// 焦点定位
							typeof (value.nextFocus) != "undefined"
									&& (value.focusInput.val().length != 0)
									&& value.nextFocus.focus();
