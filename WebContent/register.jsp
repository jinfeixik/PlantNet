...
<body>
	...
	<div class="main shadow">
		<div class="usermain">
			...
			<h1 class="title" title="用户注册">注册</h1>

			<div class="register">
				<div class="title">注册"花草网"账号</div>
				<form id="registersubmit" method="POST" action="sendEmail.form">
					<div class="list">
						<label>呢称</label> <input id="userName" name="userName" type="text"
							class="s-txt" autocomplete="off" placeholder="请输入4-6位的昵称" /> <span
							class="tip" id="userNameTip"></span>
					</div>

					<div style="margin-top: 20px;" class="list">
						<label>账号</label> <input id="userAccount" name="userAccount"
							type="text" class="s-txt" autocomplete="off"
							placeholder="请输入6到10位数字或字母" /> <span id="userAccountTip"
							class="tip"></span>
					</div>

					<div style="margin-top: 20px;" class="list">
						<label>密码</label> <input id="userPassword" name="userPassword"
							type="password" class="s-txt" autocomplete="off"
							placeholder="请输入6到10位数字或字母" /> <span id="userPasswordTip"
							class="tip"></span>
					</div>

					<div style="margin-top: 20px;" class="list">
						<label>邮箱</label> <input id="userEmail" name="userEmail"
							placeholder="请输入正确格式的可用邮箱" type="text"
							suffix="@qq.com,@sina.com,@163.com,@126.com,@souhu.com,@gmail.com"
							class="s-txt" autocomplete="off" /> <span id="userEmailTip"
							class="tip"></span>
					</div>

					<div style="margin-top: 20px;" class="list">
						<label>头像</label> <input id="userAvatar" name="avatar" type="file"
							class="s-txt" autocomplete="off" /> <span id="userAvatarTip"
							class="tip"></span>
					</div>

					<div style="margin-top: 20px;" class="list">
						<label>性别</label>男<input id="userSex" name="userSex" type="radio"
							value="男" class="s-radio" autocomplete="off" checked />女 <input
							id="" name="userSex" type="radio" value="女" class="s-radio"
							autocomplete="off" /><span id="userSexTip" class="tip"></span>
					</div>

					<div style="margin-top: 20px;" class="list">
						<label>地址</label> <input id="userAddress" name="userAddress"
							type="text" class="s-txt" autocomplete="off"
							placeholder="请输入正确地址" /> <span id="userAddressTip" class="tip"></span>
					</div>

					<div style="margin-top: 20px;" class="list">
						<label>电话</label> <input id="userPhone" name="userPhone"
							type="text" class="s-txt" autocomplete="off"
							placeholder="请输入11位数字" /> <span id="userPhoneTip" class="tip"></span>
					</div>

					<div class="sign">
						<button class="submit" type="button" onclick="registercheck()">注册</button>
					</div>
	...
	<script type="text/javascript">
		...
		$("#userName").blur(function() {
			$.ajax({
				url : "isUserNameTrue.form?userName=" + $("#userName").val(),
				data : {
				...
				success : function(data) {
					if (data == "emptymsg") {
						usernameflag = false;
						$("#userNameTip").html("昵称不能为空");
						$("#userNameTip").css("color", "red");
					}
					if (data == "existmsg") {
						usernameflag = false;
						$("#userNameTip").html("昵称已被创建");
						$("#userNameTip").css("color", "red");
					}
					if (data == "lengthmsg") {
						usernameflag = false;
						$("#userNameTip").html("昵称长度不合法");
						$("#userNameTip").css("color", "red");
					}
					if (data == "spacemsg") {
						usernameflag = false;
						$("#userNameTip").html("昵称包含空格");
						$("#userNameTip").css("color", "red");
					}
					if (data == "truemsg") {
						usernameflag = true;
						$("#userNameTip").html("昵称可用");
		...
		$("#userAccount").blur(
				function() {
					$.ajax({
						url : "isUserAccountTrue.form?userAccount="
								+ $("#userAccount").val(),
						data : {
						...
						success : function(data) {
							if (data == "emptymsg") {
								...
								$("#userAccountTip").html("账号不能为空");
								$("#userAccountTip").css("color", "red");
							}
							if (data == "existmsg") {
								useraccountflag = false;
								$("#userAccountTip").html("账号已被创建");
								$("#userAccountTip").css("color", "red");
							}
							if (data == "lengthmsg") {
								useraccountflag = false;
								$("#userAccountTip").html("账号长度不合法");
								$("#userAccountTip").css("color", "red");
							}
							if (data == "spacemsg") {
								useraccountflag = false;
								$("#userAccountTip").html("昵称存在非法字符");
								$("#userAccountTip").css("color", "red");
							}
							if (data == "truemsg") {
								useraccountflag = true;
								$("#userAccountTip").html("账号可用");
								$("#userAccountTip").css("color", "green");
							}
						},
					});
				})
		...
		$("#userPassword")
				.blur(
						function() {
							...
							} else if ($("#userPassword").val().length > 10
									|| $("#userPassword").val().length < 6) {
								userpasswordflag = false;
								$("#userPasswordTip").html("密码长度不合法");
								$("#userPasswordTip").css("color", "red");
							} else if (/^[a-zA-Z0-9]+$/.test($("#userPassword")
									.val()) == false) {
								userpasswordflag = false;
								$("#userPasswordTip").html("密码存在非法字符");
								$("#userPasswordTip").css("color", "red");
							} else {
								userpasswordflag = true;
								$("#userPasswordTip").html("密码可用");
				...
				function() {
					if ($("#userEmail").val() == '') {
						...
						$("#userEmailTip").css("color", "red");
							...
							.test($("#userEmail").val()) == false) {
						useremailflag = false;
						$("#userEmailTip").html("邮箱格式有误");
						$("#userEmailTip").css("color", "red");
					} else {
						useremailflag = true;
						$("#userEmailTip").html("邮箱可用");
						$("#userEmailTip").css("color", "green");
					}
		...
		$("#userAvatar").blur(
				function() {
					...
					if (useAvatar == "") {
						...
						$("#userAvatarTip").html("头像不能为空");
						$("#userAvatarTip").css("color", "red");
					} else if (/\.(jpg|jpeg|png|GIF|JPG|PNG)$/.test(useAvatar
							.substring(useAvatar.lastIndexOf("."),
									useAvatar.length).toLowerCase()) == false) {
						useravatarflag = false;
						$("#userAvatarTip").html("头像必须为图片格式");
						$("#userAvatarTip").css("color", "red");
					} else {
						useravatarflag = true;
						$("#userAvatarTip").html("头像可用");
						$("#userAvatarTip").css("color", "green");
					}
		...
		$("#userPhone").blur(
				function() {
					if ($("#userPhone").val() == '') {
						userphoneflag = false;
						$("#userPhoneTip").html("电话不能为空");
						$("#userPhoneTip").css("color", "red");
					} else if (/^[0-9]+$/.test($("#userPhone").val()) == false
							|| $("#userPhone").val().length != 11) {
						userphoneflag = false;
						$("#userPhoneTip").html("电话只能为十一位数字");
						$("#userPhoneTip").css("color", "red");
					} else {
						userphoneflag = true;
						$("#userPhoneTip").html("电话可用");
						$("#userPhoneTip").css("color", "green");
					}
		...
		function registercheck() {
			...
			if (usernameflag == true && useraccountflag == true
					&& userpasswordflag == true && useremailflag == true
					&& useravatarflag == true && userphoneflag == true) {

				let
				formData = new FormData($('#registersubmit')[0]);
				//提交表单
				$.ajax({
					url : "sendEmail.form",
					data : formData,
					...
					success : function(data) {
						if (data != null) {
							window.location.href = "toRegisterEmail.form";
						}
					}
				});
			}
