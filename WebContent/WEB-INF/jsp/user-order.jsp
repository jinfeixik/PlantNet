...
<title>我的订单 - 花草网</title>
<meta content="我的订单" name="keywords" />
...
<body>
								...
								<ul>
									<c:forEach items="${orderLists}" var="orderList"
										...
										<li style="height: auto;">
											<div class="w1" style="width: 760px;">
												<div style="float: left; margin-left: 20px"
													onclick="toPlantDetails(${orderList.plantDetails.plantID})">
													<i class="ico-sina-big"><img width="100px"
														height="100px"
														src="${orderList.plantDetailsImage.plantDetailsImagePath}" /></i>
												</div>
												<div style="margin-left: 100px;">
													<i style="font-size: 12px; color: #999999">订单编号:${orderList.orderCode}</i>
													<i
														style="font-size: 12px; color: #999999; margin-left: 100px">${orderList.orderCreateTime}</i>
													<a style="margin-left: 150px" onclick="">${orderList.orderState}</a>
												</div>
												<div
													style="margin-left: 80px; float: left; margin-top: 10px;">
													<p
														style="font-family: arial, sans-serif, simsun, Verdana, Helvetica; font-size: 18px; font-weight: bold; text-align: left; width: 150px; text-overflow: ellipsis; white-space: nowrap; overflow: hidden;">${orderList.plantDetails.plantDescribe}</p>
												</div>
												<div
													style="float: left; margin-left: 20px; margin-top: 10px;">
													<p
														style="font-family: arial, sans-serif, simsun, Verdana, Helvetica; font-size: 12px; text-align: left; width: 400px; text-overflow: ellipsis; white-space: nowrap; overflow: hidden; color: #999999">${orderList.plantDetailsImage.plantDetailsImageDescribe}
													</p>
												</div>
												<div style="float: left; margin-left: 80px">
													<p
														style="font-family: arial, sans-serif, simsun, Verdana, Helvetica; font-size: 12px; width: 80px; text-align: left; margin-top: 10px;">配送地址:</p>
												</div>
												<div
													style="float: left; margin-left: 10px; margin-top: 10px;">
													<p
														style="font-family: arial, sans-serif, simsun, Verdana, Helvetica; font-size: 12px; text-align: left; width: 470px; text-overflow: ellipsis; white-space: nowrap; overflow: hidden; color: #999999">
														<c:choose>
															<c:when test="${orderList.address==null}">
														未填写默认地址或地址已失效，请联系客服！
														</c:when>
															<c:otherwise>
														${orderList.address.recipient} ${orderList.address.addressPhone} ${orderList.address.province}${orderList.address.city}${orderList.address.county}${orderList.address.addressInfo}
														</c:otherwise>
														</c:choose>
													</p>
												</div>
												<div style="float: left; margin-left: 150px">
													<p
														style="font-family: arial, sans-serif, simsun, Verdana, Helvetica; font-size: 12px; width: 30px; text-align: left; margin-top: 10px;">合计:</p>
												</div>
												<div
													style="float: left; margin-left: 10px; margin-top: 10px;">
													<p
														style="font-weight: bold; font-family: arial, sans-serif, simsun, Verdana, Helvetica; font-size: 15px; text-align: left; width: 70px; text-overflow: ellipsis; white-space: nowrap; overflow: hidden; color: #F73809">￥${orderList.orderAmount}</p>
												</div>
												<div style="float: left; margin-left: 150px">
													<p
														style="font-family: arial, sans-serif, simsun, Verdana, Helvetica; font-size: 12px; text-align: left; width: 30px; margin-top: 10px;">数量:</p>
												</div>
												<div
													style="float: left; margin-left: 10px; margin-top: 10px;">
													<p
														style="font-weight: bold; font-family: arial, sans-serif, simsun, Verdana, Helvetica; font-size: 15px; text-align: left; width: 70px; text-overflow: ellipsis; white-space: nowrap; overflow: hidden; color: #F73809">${orderList.orderNumber}</p>
												</div>
												<div
													style="float: left; margin-left: 120px; margin-top: 10px;">
													<c:if test="${orderList.orderState=='待支付'}">
														<form name=alipayment id="alipayment${status.index}"
															action=alipay.trade.page.pay.jsp method=post>
															<input id="WIDout_trade_no${status.index}"
																name="WIDout_trade_no" value="${orderList.orderCode}"
																style="display: none" /> <input id="WIDsubject"
																name="WIDsubject"
																value="${orderList.plantDetails.plantDescribe}"
																style="display: none" /> <input id="WIDtotal_amount"
																name="WIDtotal_amount" value="${orderList.orderAmount}"
																style="display: none" /> <input id="WIDbody"
																name="WIDbody"
																value="${orderList.plantDetailsImage.plantDetailsImageDescribe}"
																style="display: none" /> <a
																onclick="reSubmitAlipayment(${status.index},${orderList.orderID})"
																class="btn-green-big right">继续支付</a>
														</form>
													</c:if>
													<c:if test="${orderList.orderState=='已支付'}">
														<a class="btn-grey-white right">等待发货</a>
													</c:if>
													<c:if test="${orderList.orderState=='已发货'}">
														<a onclick="ensureSend(this,${orderList.orderID})" class="btn-green-big right">确认收货</a>
													</c:if>
													<c:if test="${orderList.orderState=='已完成'}">
														<a
															onclick="toPlantDetails(${orderList.plantDetails.plantID})"
															class="btn-grey-white right">再来一单</a>
													</c:if>
												</div>
	...
	//重新提交支付
	function reSubmitAlipayment(index,orderID){
	...
	/*订单-发货*/
	function ensureSend(obj, orderID) {;
		$.ajax({
			type : 'POST',
