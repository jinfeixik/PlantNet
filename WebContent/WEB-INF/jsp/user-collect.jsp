...
<title>我的收藏 - 花草网</title>
<meta content="我的收藏" name="keywords" />
...
<body>
					...
					<div class="extend minh person-set">
						...
						<div class="set-item-info">
							<div class="bind-content">
								<ul>
									<c:forEach items="${plantCollectLists}" var="plantCollectList"
										varStatus="status">
										<li>
											<div class="w1" onclick="toPlantDetails(${plantCollectList.plantID})">
												<i class="ico-sina-big"><img width="50px" height="50px"
													src="${plantCollectList.plantImage.plantImagePath}" /></i>
											</div>
											<div class="w2" onclick="toPlantDetails(${plantCollectList.plantID})">${plantCollectList.plant.plantName}</div>
											<div class="w3" onclick="toPlantDetails(${plantCollectList.plantID})">${plantCollectList.plantCollectTime}</div>
											<div class="w4" onclick="toPlantDetails(${plantCollectList.plantID})">&nbsp;</div>
											<div class="w5">
												<a
													onclick="cancelCollect(${plantCollectList.plantID},${plantCollectList.plant.plantCollectNumber})"
													class="btn-grey-white">取消收藏</a>
											</div>
										</li>
									</c:forEach>
	...
	<script type="text/javascript">
	function cancelCollect(plantID,collectNumber){
		$.ajax({
			url : "collectPlant.form",
			data : {
				plantID : plantID,
				plantCollectNumber : (collectNumber - 1) < 0 ? 0
						: (collectNumber - 1),
				collectFlag : '取消收藏'
			},
			type : "post",
